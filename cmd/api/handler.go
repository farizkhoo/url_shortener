...
func getURLHandler(c *gin.Context) {
	...
	url, err := pg.GetURLFromUUID(req.ID)
	if err != nil {
...
func createURLHandler(c *gin.Context) {
	...
	url, err := pg.GetURLFromLongURL(req.LongURL)
	if err == nil {
		jsonCTX(c, http.StatusConflict, "url already exists in database", url.ID)
		return
	}

	if err := pg.StoreURL(req.LongURL); err != nil {
	...
	if err != nil {
		if err == sql.ErrNoRows {
			jsonCTX(c, http.StatusNotFound, "no result for url key provided", req.LongURL)
			return
		}
